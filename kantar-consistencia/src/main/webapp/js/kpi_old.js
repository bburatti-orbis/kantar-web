...
//	}
	...
	$('#agregaParam').click(function() {
		$("#agregarParam").css("display", "");
		$("#editarParam").css("display", "none");
		var cOpt = "<option value='$_valor_$'>$_glosa_$</option>";
			ModuloKpiDWR.cargaParametros({
				callback:function(resp){
					if(resp == null){
						...
						$.each(resp, function(){
							var fila = "<option value='$_valor_$'>$_glosa_$</option>";
							fila = fila.replace("$_valor_$", this.defaultS+"-"+this.idP);
	...
	function grabarParam(grabar){
		if(grabar){
			...
			fila = fila.replace("$_editar_$", "<input type='button' value='Edit' class='editar' valor="+corre+" onclick='editar("+corre+");'/> <input type='button' value='Delete' onclick='borrar("+corre+");' />");
			filas += fila;
...
//    });
	...
	function editarS(nro){
		...
		var valor = $(arrTd[2]).html();
		$("#umbral").val($(arrTd[0]).attr('idUmb'));
		$("#operador").val($(arrTd[1]).attr('idOp'));
		$('#valorSema').val(valor);
